    ...
    <div class="row">
      <div class="col-md-12">
        <div class="row">
          ...
          <div class="col card-item">
            <p>Staking APR</p>
            <div class="border-text">{{apr_value.toFixed(2)}}%</div>
          </div>
        </div>
      </div>
            ...
            >
              ...
              <option
                v-for="validator in validators.filter((val) => 
                  val.description.moniker && val.description.moniker.includes(state.filter_stake_text)
                )"
                :key="validator"
            ...
            >
              ...
              <option
                v-for="delegator in delegators_filter.filter((val) => 
                  val.name && val.name.includes(state.filter_undeligate_text)
                )"
              ...
              >
                ...
                ({{
                  ...
                  (() => {
                    return (delegator.balance.amount / 1e6).toFixed(2);
                  })()
                }}
                TMUN )
              </option>
        ...
        <div class="modal-body">
          ...
          <div>
            ...
            >
              ...
              <option
                v-for="delegator in delegators_filter.filter((val) => 
                  val.name && val.name.includes(state.filter_src_val_text)
                )"
                v-bind:key="delegator"
                :value="delegator.delegation.validator_address"
              >
        ...
        <div class="modal-body">
          ...
          <div>
            ...
            >
              ...
              <option
                v-for="delegator in delegators_filter.filter((val) => 
                 val.name && val.name.includes(state.filter_claim_text)
                )"
                v-bind:key="delegator"
                :value="delegator.delegation.validator_address"
              >
...
export interface State {
  ...
  reward_validator_address: string;
  forceUpdate: boolean;
  validadd:number;
  show_active: boolean;
  balance: number;
...
export default defineComponent ({
  ...
  data() {
    ...
    return {
      ...
      sendMsgTx: async (index: number): Promise<void> => {
        ...
        if ( index !== 4 && state.validator_address === "") {
        ...
        if (index === 4 && state.validator_address === "") {
          state.validator_address = state.reward_validator_address;
          
        }
        
        if (index === 4 && state.validator_address === "") {
          shownotify("Reward Validator Address is not valid");
          throw new Error();
        }


        
        // if (index === 4 && state.selectedvalidator === undefined)
        // state.validator_address = this.rewards_list && 
  ...
  async mounted() {
    ...
    let queryInflation = (opts: any) =>
      $s.dispatch("cosmos.mint.v1beta1/QueryInflation", opts);
    
    let queryBondedtoken = (opts: any) =>
      $s.dispatch("cosmos.staking.v1beta1/QueryPool", opts);
    
    let queryDistributionParms = (opts: any) =>
      $s.dispatch("cosmos.distribution.v1beta1/QueryParams", opts);

    const distributionparams = await queryDistributionParms({
      params: { status: "true" },
      options: { subscribe: true },
    });
    
    const bondedtoken = await queryBondedtoken({
      params: { status: "true" },
      options: { subscribe: true },
    });
      const inflations = await queryInflation({
      params: { status: "true" },
      options: { subscribe: true },
    });
    
    
    const total_supply = await queryBankTotal({
      params: { status: "true" },
      options: { subscribe: true },
    });
    let total_supply_tmun_amount = 0
    for(let i = 0; i < total_supply.supply.length; i++)
    {
      if(total_supply.supply[i].denom == 'utmun')
        total_supply_tmun_amount += total_supply.supply[i].amount
    }
    this.apr_value = total_supply_tmun_amount * (1 - distributionparams.params.community_tax) * inflations.inflation/bondedtoken.pool.bonded_tokens * 100;
    ...
    if(address.value != "")
      state.balance = balances_new?.balances[0]?.amount || '0';
    ...
    this.validators = res.validators.sort(function(a, b){
    ...
    state.reward_validator_address = this.rewards_list.rewards[0]?.validator_address;
    ...
    state.max_value = this.delegators_filter[0]?.balance.amount / 1e6 || 0;
